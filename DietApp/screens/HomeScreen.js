...
export default function HomeScreen() {
  ...
  const SERVER_IP = '192.168.1.108'; // Sunucu IP adresinizi buraya girin
  ...
  useEffect(() => {
    const searchFood = async () => {
      ...
      setSearching(true);
      try {
        const response = await axios.get(`${API_URL}/ara-yemek?q=${searchText}`);
        setSearchResults(response.data.sonuclar || []);
      } catch (error) {
    ...
    const timer = setTimeout(() => {
      if (searchModalVisible) searchFood();
    }, 300); // 300ms debounce
  ...
  useEffect(() => {
    const searchSport = async () => {
      ...
      setSearching(true);
      try {
        const response = await axios.get(`${API_URL}/ara-spor?q=${searchText}`);
        setSearchResults(response.data.sonuclar || []);
      } catch (error) {
        console.log('Spor arama hatası:', error);
        setSearchResults([]);
      }
      setSearching(false);
    };
    ...
    const timer = setTimeout(() => {
      if (sportModalVisible) searchSport();
    }, 300);

    return () => clearTimeout(timer);
  }, [searchText, sportModalVisible]);
  ...
  const deleteFood = (item) => {
    Alert.alert("Silinsin mi?", "Silinecek.", [{ text: "İptal", style: "cancel" }, {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      <StatusBar barStyle={theme.background === '#f8f9fa' ? "dark-content" : "light-content"} backgroundColor={theme.background} />

      <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', paddingVertical: 10, backgroundColor: theme.cardBg }}>
        <TouchableOpacity onPress={() => changeDate(-1)} style={{ padding: 10 }}><Ionicons name="chevron-back" size={24} color={theme.text} /></TouchableOpacity>
        <Text style={{ fontSize: 18, fontWeight: 'bold', color: theme.text, marginHorizontal: 20 }}>{formatDateForDisplay(selectedDate)}</Text>
        <TouchableOpacity onPress={() => changeDate(1)} style={{ padding: 10, opacity: selectedDate.toDateString() === new Date().toDateString() ? 0.3 : 1 }} disabled={selectedDate.toDateString() === new Date().toDateString()}><Ionicons name="chevron-forward" size={24} color={theme.text} /></TouchableOpacity>
      </View>

      <View style={{ padding: 20, backgroundColor: theme.cardBg, borderBottomLeftRadius: 20, borderBottomRightRadius: 20, elevation: 5 }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
          <Text style={{ fontSize: 24, fontWeight: 'bold', color: theme.text }}>Özet</Text>
      ...
      <ScrollView style={{ flex: 1, padding: 15 }}>
        {groupedLogs['Spor Aktiviteleri'].length > 0 && (<View style={{ marginBottom: 20 }}><Text style={{ fontSize: 18, fontWeight: 'bold', color: theme.text, marginBottom: 10 }}>Spor Aktiviteleri 🏃‍♂️</Text>{groupedLogs['Spor Aktiviteleri'].map((item, index) => (<TouchableOpacity key={index} style={[styles.logItem, { borderLeftWidth: 4, borderLeftColor: '#27ae60', backgroundColor: theme.cardBg }]} onLongPress={() => deleteFood(item)}><View style={styles.iconContainer}><Ionicons name="flame" size={20} color="#e74c3c" /></View><View style={styles.infoContainer}><Text style={[styles.foodName, { color: theme.text }]}>{item.aktivite_adi}</Text><Text style={{ fontSize: 12, color: theme.textSecondary }}>{item.sure_dk} dakika</Text></View><Text style={[styles.calorieText, { color: '#27ae60' }]}>-{item.kalori}</Text></TouchableOpacity>))}</View>)}
        {['Kahvaltı', 'Öğle Yemeği', 'Akşam Yemeği', 'Atıştırmalık'].map((mealType) => (
          <View key={mealType} style={{ marginBottom: 20 }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10, alignItems: 'center' }}><Text style={{ fontSize: 18, fontWeight: 'bold', color: theme.text }}>{mealType}</Text><TouchableOpacity onPress={() => openSearchForMeal(mealType)}><Ionicons name="add-circle" size={28} color="#2ecc71" /></TouchableOpacity></View>
            {(groupedLogs[mealType] && groupedLogs[mealType].length > 0) ? (groupedLogs[mealType].map((item, index) => (<TouchableOpacity key={index} style={[styles.logItem, { backgroundColor: theme.cardBg }]} onLongPress={() => deleteFood(item)}><View style={styles.iconContainer}><Ionicons name="fast-food" size={20} color="#f39c12" /></View><View style={styles.infoContainer}><Text style={[styles.foodName, { color: theme.text }]}>{item.yemek_adi}</Text><Text style={{ fontSize: 12, color: theme.textSecondary }}>{item.porsiyon}</Text></View><Text style={styles.calorieText}>{item.kalori}</Text></TouchableOpacity>))) : (<View style={{ backgroundColor: theme.cardBg, padding: 15, borderRadius: 10, alignItems: 'center', borderStyle: 'dashed', borderWidth: 1, borderColor: theme.border }}><Text style={{ color: theme.textSecondary }}>Boş</Text></View>)}
          </View>
        ))}
      ...
      <Modal animationType="fade" visible={searchModalVisible} onRequestClose={() => setSearchModalVisible(false)}>
        <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
          <View style={{ padding: 20, flexDirection: 'row', alignItems: 'center', borderBottomWidth: 1, borderColor: '#f1f2f6' }}>
            <TouchableOpacity onPress={() => setSearchModalVisible(false)}>
              <Ionicons name="arrow-back" size={24} color="#34495e" />
            </TouchableOpacity>
            <TextInput
              style={{ flex: 1, marginLeft: 15, fontSize: 18, backgroundColor: '#f5f6fa', padding: 10, borderRadius: 10 }}
              placeholder={`${currentMealType} için ara... (Örn: Risotto)`}
              value={searchText}
      ...
      <Modal animationType="fade" visible={sportModalVisible} onRequestClose={() => setSportModalVisible(false)}>
        <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
          <View style={{ padding: 20, flexDirection: 'row', alignItems: 'center', borderBottomWidth: 1, borderColor: '#f1f2f6' }}>
            ...
            <TextInput
              style={{ flex: 1, marginLeft: 15, fontSize: 18, backgroundColor: '#f5f6fa', padding: 10, borderRadius: 10 }}
              placeholder="Spor ara... (Örn: Yüzme)"
              value={searchText}
              onChangeText={setSearchText}
              autoFocus
            />
          </View>
          <View style={{ padding: 20, backgroundColor: '#f9f9f9', borderBottomWidth: 1, borderColor: '#eee' }}>
            <Text style={{ marginBottom: 10, fontWeight: 'bold', color: '#2c3e50' }}>Süre (Dakika):</Text>
            <TextInput style={{ backgroundColor: '#fff', padding: 10, borderRadius: 10, borderWidth: 1, borderColor: '#ddd' }} placeholder="Örn: 30" keyboardType="numeric" value={duration} onChangeText={setDuration} />
          </View>
